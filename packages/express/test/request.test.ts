...
import { ExpressRequestAdapter } from '../src/request';
...
describe('ExpressRequestAdapter', () => {
  describe('constructor', () => {
    it('should create instance with Express request', () => {
      const mockReq = createMockRequest();
      const adapter = new ExpressRequestAdapter(mockReq);

      expect(adapter).toBeInstanceOf(ExpressRequestAdapter);
    });
  });
  ...
  describe('cookies getter', () => {
    ...
    it('should handle undefined cookies', () => {
      const mockReq = createMockRequest({ cookies: undefined });
      const adapter = new ExpressRequestAdapter(mockReq);

      expect(adapter.cookies).toBeUndefined();
    });
  });
  ...
  describe('params getter', () => {
    ...
    it('should cast params to Record<string, string>', () => {
      const params = {
        id: '123', // Express params are strings
        slug: 'test-article',
      };
      const mockReq = createMockRequest({ params });
      const adapter = new ExpressRequestAdapter(mockReq);
