...
export async function updateUser(username: string, updateUser: User) {
  const user = await userUpdate(username, updateUser);
  if (!user) {
...
export async function getProfile(currentUser: User, username: string) {
  const user = await userGet(username);
  if (!user) {
    throw new UserNotFoundError('User not found');
  }
